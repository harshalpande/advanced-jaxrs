...
public class ShortDateMessageBodyWriter implements MessageBodyWriter<Date> {
	...
	public void writeTo(Date date, Class<?> classType, Type type, Annotation[] listAnnotation, MediaType mediaType,
		...
		cal.setTime(date);
		String shortdate = String.valueOf(cal.get(Calendar.DATE)).concat("-").concat(String.valueOf(cal.get(Calendar.MONTH + 1)).concat("-").concat(String.valueOf(cal.get(Calendar.YEAR))));
		os.write(shortdate.toString().getBytes());
